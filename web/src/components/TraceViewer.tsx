...
export const TraceViewer: React.FC<TraceViewerProps> = ({ trace, onClose }) => {
  ...
  return (
    <div className="bg-[#161b22] border border-[#30363d] rounded-md p-5 space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between pb-3 border-b border-[#21262d]">
        <div>
          <h3 className="text-sm font-mono font-bold text-[#f0f6fc] flex items-center">
            <Hash className="w-4 h-4 mr-1.5 text-[#58a6ff]" />
            Forensic Lineage &amp; Verification Trace
          </h3>
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="text-xs font-mono text-[#8b949e] hover:text-[#c9d1d9] px-2 py-1 rounded bg-[#21262d]"
          >
            Close
          </button>
        )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {trace.processing_history?.map((step, idx) => (
            <React.Fragment key={idx}>
              <div className="flex items-center space-x-1.5 bg-[#0d1117] border border-[#30363d] px-3 py-1.5 rounded-md text-xs font-mono">
                {step.result === 'failed' ? (
                  <AlertTriangle className="w-3.5 h-3.5 text-rose-400" />
                ) : (
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                )}
                <span className="font-semibold text-[#f0f6fc]">{step.stage}</span>
                <span className="text-[#8b949e]">({step.result})</span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#0d1117] border border-[#30363d] rounded-md p-4 flex flex-col justify-between space-y-3">
          <div>
            <div className="flex items-center justify-between pb-1 border-b border-[#21262d]">
              <span className="text-xs font-mono uppercase font-bold text-[#58a6ff]">
                Normalized Event Integrity
              </span>
              <Badge variant="blue">-verify-event</Badge>
            </div>
            <div className="text-[11px] font-mono text-[#8b949e] mt-1.5 truncate">
              ID: {trace.event_id}
            </div>
          </div>
          ...
          >
            ...
            {verifyingEvent ? 'Verifying Event...' : 'Verify Normalized Event'}
          </button>
        ...
        <div className="bg-[#0d1117] border border-[#30363d] rounded-md p-4 flex flex-col justify-between space-y-3">
          <div>
            ...
            <div className="text-[11px] font-mono text-emerald-400 mt-1.5 truncate font-semibold">
          ...
          >
            <ShieldCheck className="w-3.5 h-3.5 mr-1.5" />
            {verifyingRaw ? 'Verifying Raw...' : 'Verify Raw Wire Bytes'}
          </button>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex space-x-1 bg-[#0d1117] p-0.5 rounded border border-[#30363d]">
            <button
              onClick={() => setViewMode('text')}
              className={`px-2 py-0.5 text-xs font-mono rounded ${
                viewMode === 'text' ? 'bg-[#21262d] text-[#58a6ff]' : 'text-[#8b949e]'
              }`}
            >
            ...
            <button
              ...
              className={`px-2 py-0.5 text-xs font-mono rounded ${
                viewMode === 'hex' ? 'bg-[#21262d] text-[#58a6ff]' : 'text-[#8b949e]'
              }`}
            >
        ...
        <pre className="bg-[#0d1117] border border-[#21262d] p-3 rounded-md text-xs font-mono text-[#c9d1d9] overflow-x-auto max-h-60 leading-relaxed">
      ...
      {trace.unmapped && Object.keys(trace.unmapped).length > 0 && (
        <div>
          ...
          <pre className="bg-[#0d1117] border border-amber-900/50 p-3 rounded-md text-xs font-mono text-amber-200/90 overflow-x-auto max-h-48">
      ...
      <div>
        ...
        <pre className="bg-[#0d1117] border border-[#21262d] p-3 rounded-md text-xs font-mono text-[#c9d1d9] overflow-x-auto max-h-64">
